...
void SHADER::Init(IDirect3DDevice9 *Dev, const char fName[], int typ)
{
    ...
    if (m_type == PIXEL_SHADER)
        ...
        hRes = D3DXCompileShaderFromFile(fName, NULL, NULL, "Main", "vs_2_0", D3DXSHADER_DEBUG, &Code, &ErrorMsgs, &m_pConstantTable);
    ...
    if ((FAILED(hRes)) && (ErrorMsgs != NULL)) // If failed
    {
        if (m_type == PIXEL_SHADER)
            ...
            debug.Print("Couldnt compile the Vertex Shader");
    ...
    if (m_type == PIXEL_SHADER)
        ...
        hRes = m_pDevice->CreateVertexShader((DWORD *)Code->GetBufferPointer(), &m_pVertexShader);
    ...
    if (FAILED(hRes))
    {
        if (m_type == PIXEL_SHADER)
            ...
            debug.Print("Couldnt Create the Vertex Shader");
        exit(0);
    }
}
...
void SHADER::Begin()
{
    if (m_type == PIXEL_SHADER)
        ...
        HRESULT hRes = m_pDevice->SetVertexShader(m_pVertexShader);
}
...
void SHADER::End()
{
    if (m_type == PIXEL_SHADER)
        ...
        HRESULT hRes = m_pDevice->SetVertexShader(NULL);
}
